...
def extend_non_parametric_labels(labeled_points, labels, unlabeled_points, sigma):
    ...
    for row in range(N):
        distances[:,row] = np.exp(-np.linalg.norm(labeled_points - unlabeled_points[row], axis = 1) ** 2 / (sigma ** 2))

    predictions = np.sum(distances * np.expand_dims(labels, axis = 1), axis = 0)/np.sum(distances, axis=0)
    return predictions
...
def non_parametric_approx(W, y, main_points, extended_points, num_labeled, Lambda, sigma, inv_function):
    ...
    time3 = time.time()
    solution_main = find_non_parametric_labels(W, np.copy(y_main), num_labeled, Lambda, inv_function)
    time4 = time.time()
    
    time5 = time.time()
    extended_solution = extend_non_parametric_labels(main_points, solution_main, extended_points, sigma)
    time6 = time.time()

    time2 = time.time()

    accuracy_main, incorrect_main = util.measure_accuracy(solution_main, y_main, .5)
    accuracy_extended, incorrect_extended = util.measure_accuracy(extended_solution, y_extended, .5)

    return accuracy_main, incorrect_main, accuracy_extended, incorrect_extended, time2 - time1

def approx_feedback_set(W, Y_l, main_points, extended_points, sigma_0, Lambda, inv_function, eps=1e-3, delta=1, unlabeled_subset=None, kNN=None):
    ...
    for x in unlabeled_subset:
        ...
        while sigma < max_sigma and sigma > min_sigma:
            ...
            d_tilde_f_dsigma = (np.sum(dW_x_dsigma * f_l) + np.sum(W_x * df_dsigma) + f_u * np.sum(dW_x_dsigma)) / total_distance

            dgu_dsigma = 2 * (f_u - .5) * d_tilde_f_dsigma

            # step closer to our solution
            Nesterov  = delta * dgu_dsigma
            Newton = float("inf")
            if dgu_dsigma != 0: Newton = g_u / dgu_dsigma
...
def find_accuracy_values(sigma_0, sigma_end, Lambda, W, Y, main_points, extended_points, num_labeled, inv_function, problem_instance_function, step_size=.05, kNN=None):
    ...
    arr_main, arr_extended, incorrect_extended_list = np.zeros(val_range.shape), np.zeros(val_range.shape), []
    ...
    for index, sigma in enumerate(val_range):
        ...
        accuracy_main, incorrect_main, accuracy_extended, incorrect_extended, time = problem_instance_function(sigma_matrix_array_inverse, Y,  main_points, extended_points, num_labeled, Lambda, sigma, inv_function)
...
def plot_all_approx_feedback_sets(sigma_0,sigma_end, intervals, ground_truth_accuracys, extended_accuracys, step_size=.05, incorrect_extended=None, path=None, title=None, images_path=None, intervals_path=None, time=None, overallcount = -1):
    ...
    assert val_range.shape == ground_truth_accuracys.shape and val_range.shape == extended_accuracys.shape, "Accuracys should have same shape as arange(sigma_0, sigma_end, step_size), where step size defaults to .05"
    ...
    y_vals = [.005 + extended_accuracys[np.argmin(np.array([abs(errorval - val) for val in val_range]))] for errorval in errorVals]
    y_vals = [y_val if y_val != float("inf") else sigma_end for y_val in y_vals]

    biglist = [util.errorbar(errorVals[x], y_vals[x], errorsizes[x], .004) for x in range(len(errorsizes)) if errorVals[x] != float('inf') and errorVals[x] != -float("inf")]
    ...
    if intervals_path is not None:
        intervals_dict = {"ground_truth_extended":list(extended_accuracys), "ground_truth_subset": list(ground_truth_accuracys), "sigma_vals": list(val_range), "intervals": intervals, "incorrect": incorrect_extended, "time": time, "count": overallcount}
