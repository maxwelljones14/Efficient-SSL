...
def harmonic_approx(W_sigma, Y_l, inv_function):
    ...
    if sp.issparse(W_sigma):
        ...
        P_uu_row_normalized, P_ul_row_normalized = (
            P[num_labeled:, num_labeled:],
            P[num_labeled:, 0:num_labeled],
        )
...
def approx_feedback_set(
    ...
    if sparse:
        ...
        sparse_neighbors = sp.coo_matrix(
            W_low + np.eye(W.shape[0])
        )  # adding the identity so that the shape of the sparse neighbor graph is correct
    ...
    for x in unlabeled_subset:
        ...
        while sigma < max_sigma and sigma > min_sigma:
            ...
            if sparse:
                sparse_neighbors_sigma = (
                    -(sparse_neighbors.multiply(sparse_neighbors) / sigma**2)
                ).expm1()
                ...
                P_uu_row_normalized, P_ul_row_normalized = (
                    P_sparse[num_labeled:, num_labeled:],
                    P_sparse[num_labeled:, 0:num_labeled],
                )
                I_minus_P_uu = sp.eye(num_unlabeled) - P_uu_row_normalized
            else:
            ...
            if sparse:
                ...
                dP_dsigma = sp.csr_matrix(
                    (dw_dsigma - P_sparse.multiply(dw_dsigma_row_sums)).multiply(1 / W_row_sums)
                )
            else:
            ...
            try:
                df_dsigma = inv_function(
                    I_minus_P_uu,
                    dP_dsigma[num_labeled:, num_labeled:] @ f_u
                    + dP_dsigma[num_labeled:, 0:num_labeled] @ f_l,
                )
            except:
                print(f"Singular matrix")
                break
            dgu_dsigma = 2 * (f_u - 0.5) * df_dsigma

            # step closer to our solution
            Nesterov = delta * dgu_dsigma[x]
            Newton = float("inf")
            if dgu_dsigma[x] != 0:
                Newton = 2 * g_u[x] / dgu_dsigma[x]
...
def find_all_approx_feedback_sets(
    ...
    while sigma < sigma_end:
        ...
        min, maxVal, count = interval_function(
            W, Y[0:num_labeled], sigma, inv_function, epsilon, delta, kNN=kNN, sparse=sparse
        )
        time2 = time.time()
        num_intervals += 1
    ...
    for x in range(len(intervals)):
        ...
        if left < sigma_0:
            intervals[x] = (sigma_0, intervals[x][1])
        if right > sigma_end:
            intervals[x] = (intervals[x][0], sigma_end)
    intervals = [intervals[0]] + [
        (max(intervals[index][0], intervals[index - 1][1]), intervals[index][1])
...
def find_accuracy_values(
    ...
    for index, sigma in enumerate(val_range):
        ...
        solution, time = problem_instance_function(
            sigma_matrix_array_inverse, Y[0:num_labeled], inv_function
        )
...
def plot_all_approx_feedback_sets_with_accuracy(
    sigma_0,
    sigma_end,
    intervals,
    ...
    ), "Ground truth accuracys should have same shape as arange(sigma_0, sigma_end, step_size), where step size defaults to .05"
    ...
    y_vals = [
        0.005
        + ground_truth_accuracys[np.argmin(np.array([abs(errorval - val) for val in val_range]))]
        for errorval in errorVals
    ]
    y_vals = [y_val if y_val != float("inf") else sigma_end for y_val in y_vals]

    biglist = [
        util.errorbar(errorVals[x], y_vals[x], errorsizes[x], 0.005) for x in range(len(errorsizes))
    ]
    ...
    intervals_dict = {
        "ground_truth": list(ground_truth_accuracys),
...
def parse_args():
    ...
    if args.inverse_type == "CG":
        ...
        if args.kNN is not None:
            args.sparse = True
            print(
                "kNN graph with CG method toggled on - doing all interval calculation with sparse matricies"
            )
        else:
            args.sparse = False
    else:
