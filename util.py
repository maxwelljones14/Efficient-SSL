...
import ssl 
ssl._create_default_https_context = ssl._create_unverified_context #in order to download USPS dataset

def inplace_csr_row_normalize_l1(X):
    """Inplace row normalize using the l1 norm"""
    _inplace_csr_row_normalize_l1(X.data, X.shape, X.indices, X.indptr)

def _inplace_csr_row_normalize_l1(X_data,
                                  shape,
                                  X_indices,
                                  X_indptr):
    n_samples = shape[0]
...
def errorbar(midpointX, midpointY, distance, height):
    ...
    return [[(midpointX - distance, midpointY),(midpointX + distance, midpointY)], \
            [(midpointX - distance, midpointY + height),(midpointX - distance, midpointY - height)], \
            [(midpointX + distance, midpointY + height),(midpointX + distance, midpointY - height)]]
...
def measure_accuracy(guess_arr,actual_arr, threshold=.5):
...
def load_pytorch_dataset(name):
    ...
    if name == "MNIST":
        trainset = torchvision.datasets.MNIST(root='/datasets/', download=True, train=True, transform=transform)
    elif name == "FashionMNIST":
        trainset = torchvision.datasets.FashionMNIST(root='/datasets/', download=True, train=True, transform=transform)
    elif name == "CIFAR":
        transform = transforms.Compose([ToTensor(),Normalize([0.4914, 0.4822, 0.4465], [0.2471, 0.2435, 0.2616])])
        trainset = torchvision.datasets.CIFAR10(root='/datasets/', download=True, train=False, transform=transform)
    elif name == "USPS":
        trainset = torchvision.datasets.USPS(root="/datasets/", download=True, train=True, transform=transform)
    else:
        trainset = torchvision.datasets.Omniglot(root='/datasets/', download=True, transform=transform)
    ...
    if name == "Omniglot":
        y_train = [trainset._characters[character_class].split("\\")[0] for image_name, character_class in trainset._flat_character_images]
        curr_number = 0
...
def setup_delalleau_experiment(dataset, subset_size, extension_size, labels_subset=None, num_subsets=1, PCA=False, PCA_n_components=None, seed=0):
    ...
    values_main, values_extended = np.zeros((num_subsets, subset_size, datapoint_size)), np.zeros((num_subsets, extension_size, datapoint_size))

    for subset in range(num_subsets):
        np.random.seed(seed + subset)
        shuffle = np.arange(n)
        np.random.shuffle(shuffle)
        curr_subset_X, curr_subset_Y, curr_subset_extended = np.copy(X[shuffle][0:subset_size]), np.copy(Y[shuffle][0:subset_size + extension_size]),  np.copy(X[shuffle][subset_size:subset_size + extension_size])
...
def get_random_subsets(dataset, subset_size, labels_subset=None, num_subsets=1, PCA=False, PCA_n_components=None, seed=0):
    ...
    n = X.shape[0]
    results_X, results_Y = np.zeros((num_subsets, subset_size, subset_size)), np.zeros((num_subsets, subset_size))

    for subset in range(num_subsets):
        np.random.seed(seed + subset)
        shuffle = np.arange(n)
        np.random.shuffle(shuffle)
        curr_subset_X, curr_subset_Y = np.copy(X[shuffle][0:subset_size]), np.copy(Y[shuffle][0:subset_size])
        results_X[subset] = create_matrix_from_training_data(curr_subset_X, l2_dist)
        results_Y[subset] = curr_subset_Y
